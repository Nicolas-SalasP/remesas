...
if (!isset($_SESSION['user_id'])) {
    ...
    die("Acceso denegado. Debes iniciar sesión para generar un comprobante.");
}

$transactionId = $_GET['id'] ?? null;
if (!is_numeric($transactionId) || $transactionId <= 0) {
    http_response_code(400);
    die("ID de transacción no válido o no proporcionado.");
}
...
if (!$conexion) {
     error_log("Error crítico: No se pudo establecer la conexión a la base de datos en generar-factura.php");
...
if (!$stmt) {
     error_log("Error al preparar la consulta para generar factura (TX ID: {$transactionId}): " . $conexion->error);
     http_response_code(500);
     die("Error interno al preparar la consulta de la transacción.");
}

$stmt->bind_param("ii", $transactionId, $userId);

if (!$stmt->execute()) {
     error_log("Error al ejecutar la consulta para generar factura (TX ID: {$transactionId}): " . $stmt->error);
     http_response_code(500);
     die("Error interno al buscar la transacción.");
}
...
if (!$tx) {
    ...
    die("Transacción no encontrada o no pertenece a este usuario.");
}
...
try {
    ...
    error_log("Error crítico al generar PDF para TX ID {$transactionId}: " . $e->getMessage());
    http_response_code(500);
    die("Lo sentimos, ocurrió un error interno al generar el comprobante PDF. Por favor, contacta a soporte.");
}
?>